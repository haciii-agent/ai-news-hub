...
async function loadRecommendations(append = false) {
  ...
  try {
    const res = await recommendationsApi.list({ page: currentPage.value, per_page: PER_PAGE })
    const data = res.data
...
.article-meta { display: flex; gap: 6px; font-size: 13px; color: var(--text-secondary); margin-bottom: 8px; align-items: center; }
